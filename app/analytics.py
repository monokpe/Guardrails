...
def get_analytics_stats(
    ...
    usage_query = db.query(models.TokenUsage).filter(
        models.TokenUsage.tenant_id == tenant_id,
        models.TokenUsage.timestamp >= start_date
    )
    ...
    # Security stats
    # Injection > 0.5
    injection_count = logs_query.filter(
        models.AuditLog.injection_score > 0.5
    ).count()
    ...
    # Avg latency
    avg_latency = logs_query.with_entities(
        func.avg(models.AuditLog.latency_ms)
    ).scalar() or 0.0
...
def get_analytics_timeseries(
    ...
    for d in sorted_dates:
        stats = data_map[d]
        result_data.append(TimeSeriesPoint(
            date=d,
...
def get_violations_breakdown(
    ...
    pii_types = [
        ViolationType(type=k, count=v) 
        for k, v in pii_counts.items()
    ]
    
    injection_types = [
        ViolationType(type=k, count=v)
        for k, v in injection_counts.items()
        if v > 0
    ]
    
    return ViolationsBreakdownResponse(
        pii_types=pii_types,
