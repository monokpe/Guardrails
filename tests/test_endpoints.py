...
def test_analyze_endpoint_success(client, db_session):
    ...
    db_session.commit()
    customer = Customer(name="Endpoint Test Customer", email="endpoint@example.com", tenant_id=tenant.id)
...
def test_rate_limiting(client, db_session):
    ...
    db_session.add(tenant)
    db_session.commit()
    customer = Customer(name="Rate Limit Test Customer", email="ratelimit@example.com", tenant_id=tenant.id)
    db_session.add(customer)
    db_session.commit()
    db_session.refresh(customer)
...
def test_analyze_endpoint_invalid_input(client, db_session, invalid_payload):
    ...
    db_session.add(tenant)
    db_session.commit()
    customer = Customer(name="Invalid Input Test Customer", email="invalidinput@example.com", tenant_id=tenant.id)
    db_session.add(customer)
    db_session.commit()
    db_session.refresh(customer)
